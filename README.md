...
12. [Database migrations](#database-migrations)
    ...
    - [Catch-up migrations](#catch-up-migrations)
13. [Use Storybook for an interface overview](#use-storybook-for-an-interface-overview)
14. [Linting and Testing](#linting-and-testing)
    - [Backend](#backend)
    - [Frontend](#frontend)
...
| `update:start`                                                      | `PS_AUTOUP_UNINSTALL_NON_COMPAT_MODS`         | `--uninstall-incompatible-modules` | `true` (default), `false`, `'true'`, `'false'`, `'1'`, `'0'`, `1`, `0`, `'on'`, `'off'` | True by default. If enabled, uninstall all incompatible modules before the update, reducing the risk of failed update.                  |
...
7. **UpdateDatabase**: This step runs all the [database migrations](#database-migrations) between the source and the destination version. Then, it will run some additional steps, such as cache deletion, language update…
...
The database changes applied by the **UpdateDatabase** step are described by migration classes stored in [`upgrade/migrations/`](/upgrade/migrations). The layout is one folder per PrestaShop version, one class per topic:

```
upgrade/migrations/
├── 9.0.2/
│   ├── 0010_SessionSetup.php
│   ├── 0020_CustomerMessages.php
│   ├── 0030_AuthorizationRoles.php
│   └── 0040_Hooks.php
└── 9.0.3/
    └── ...
```

A migration is applied when its version (the folder name) is **strictly greater** than the version the shop is updated from, and **lower than or equal to** the version the shop is updated to. Versions are compared with PHP's `version_compare()`.

### Adding a migration

1. Pick the folder of the PrestaShop version your change ships in, or create it if it is the first change for that version (e.g. `upgrade/migrations/9.2.1/`).
2. Create a file named `<order>_<Topic>.php`, where `<order>` is a number defining the position inside the version (see [Execution order](#execution-order)) and `<Topic>` is the class name, in StudlyCaps, describing the feature or tables concerned.
3. Write the class: it must be named after the file (without the numeric prefix), extend `AbstractMigration`, and live in the namespace of its version folder (`Versions\Version_<version with dots and dashes replaced by underscores>`):

```php
<?php

namespace PrestaShop\Module\AutoUpgrade\Migrations\Versions\Version_9_2_1;

use PrestaShop\Module\AutoUpgrade\Migrations\AbstractMigration;

class CustomerMessages extends AbstractMigration
{
    protected function up(): void
    {
        // https://github.com/PrestaShop/PrestaShop/pull/12345
        $this->addSql('ALTER TABLE `PREFIX_customer_message` MODIFY `user_agent` varchar(255) DEFAULT NULL');

        $this->addPhpFunction('add_column', ['customer_message', 'read_at', 'DATETIME DEFAULT NULL']);
    }
}
```
...
- `addSql(string $query)`: a single SQL query. The placeholders `PREFIX_` (table prefix), `ENGINE_TYPE` (MySQL engine) and `DB_NAME` (database name) are replaced at execution time.
- `addPhpFunction(string $functionName, array $parameters = [])`: a call to a function defined in `upgrade/php/<functionName>.php`, for changes that need logic (conditions, data transformation…). Parameters must be scalar values or arrays, as they are serialized between two steps of the update process.

### Execution order

Migrations run **version by version** (natural order), and **file by file** inside a version (natural order of file names, which the numeric prefix controls).

The prefixes advance by steps of 10 (`0010`, `0020`, `0030`…) so that a migration can be inserted anywhere relative to the others without renaming anything: to run a migration between `0020_...` and `0030_...`, name it `0025_...`. Operations inside a class run in the order they are registered in `up()`.

### Writing rules

- **One version never runs twice.** A shop that is already on 9.0.2 will not run the 9.0.2 migrations when updating to 9.0.3. If a change must reach shops that already passed the version, see [Catch-up migrations](#catch-up-migrations).
- **Each operation must stand on its own.** The update process can stop and resume between any two operations (timeouts, page reloads), and each operation may run on a fresh database connection. Do not rely on session state (`SET SESSION`, user variables) or on PHP state shared between two operations.
- **Failures do not stop the update.** Failed queries are logged and flagged as warnings, and "already exists" MySQL errors are silently ignored, which keeps migrations tolerant to being partially re-applied. Write idempotent statements whenever possible.
- **`CREATE TABLE` drops first.** The runner issues a `DROP TABLE IF EXISTS` before any `CREATE TABLE` operation, recreating the table from scratch.
- **Test both paths** with `/autoupgrade-migration-check`: a store updated through your migration and a fresh install of the destination version must end up with the same schema.

### Catch-up migrations

When a change must also be applied to shops that already run the version it belongs to (e.g. a fix added to the 9.0.x branch after 9.0.1 was released), it goes in a `<version>-catchup` folder (e.g. `9.0.1-catchup/`).

Because `version_compare('9.0.1-catchup', '9.0.1')` is negative, these migrations run right after the ones of `9.0.1` for shops coming from an older version, but are **skipped** by shops already on 9.0.1 or higher — those get the change from the regular migration of a newer version, where it must also be added. A catch-up folder therefore always mirrors changes listed in a newer version's migrations.
