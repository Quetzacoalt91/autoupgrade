...
class ColumnSizeAndIndexUpdates extends AbstractMigration
{
    protected function up(): void
    {
        ...
        $this->addSql('DELETE FROM `PREFIX_configuration` WHERE `name` = \'PS_PRICE_DISPLAY_PRECISION\'');

        /*
         * Set optin field value to 0 in employee table
         */
        $this->addSql('ALTER TABLE `PREFIX_employee` MODIFY COLUMN `optin` tinyint(1) unsigned DEFAULT NULL');

        // Increase column size
        $this->addSql('UPDATE `PREFIX_hook` SET `name` = SUBSTRING(`name`, 1, 191)');
        ...
        $this->addPhpFunction('ps_1770_update_charset');
        $this->addSql('UPDATE `PREFIX_alias` SET `alias` = SUBSTRING(`alias`, 1, 191)');
